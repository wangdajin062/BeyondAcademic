      ...
      <div className="panel-header">
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="toolbar-btn" onClick={() => fileInputRef.current?.click()}
            disabled={importing} style={{ padding: '4px 10px', fontSize: 12 }}>
            {importing ? '导入中...' : '导入'}
          </button>
          <button className="toolbar-btn" onClick={handleClearAll}
            disabled={refs.length === 0} style={{ padding: '4px 10px', fontSize: 12 }}>
            清空
          </button>
          <input ref={fileInputRef} type="file" accept=".ris,.bib"
            style={{ display: 'none' }} onChange={handleImport} />
        </div>
      </div>
      <div className="panel-content">
        {error && (
          <div style={{ padding: '8px 12px', margin: '0 12px', background: '#e74c3c', borderRadius: 6, fontSize: 12, color: '#fff', marginTop: 8 }}>
            {error}
          </div>
        )}
        {loading ? (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '40px 20px', fontSize: 13 }}>加载中...</div>
        ) : refs.length === 0 ? (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '40px 20px', fontSize: 13 }}>
            <div style={{ fontSize: 32, marginBottom: 12, opacity: 0.3 }}>📄</div>
            <p>暂无文献</p>
            <p style={{ fontSize: 12, marginTop: 8 }}>点击「导入」按钮上传 .ris 或 .bib 文件</p>
          </div>
        ) : (
          <div style={{ padding: '0 12px' }}>
            ...
            {refs.map((ref) => (
              <div key={ref.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border-light)', fontSize: 13 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {ref.doi && <span style={{ fontSize: 11, color: 'var(--accent)' }}>DOI: {ref.doi}</span>}
                  <button onClick={() => handleDelete(ref.id)}
                    style={{ fontSize: 11, color: '#e74c3c', background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginLeft: 'auto' }}>
                    删除
                  </button>
                </div>
              </div>
        ...
        <div style={{ margin: '12px', padding: '12px', background: 'var(--bg-tertiary)', borderRadius: 8, fontSize: 12, lineHeight: 1.6, color: 'var(--text-secondary)' }}>
