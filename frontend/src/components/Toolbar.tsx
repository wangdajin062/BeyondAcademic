...
export default function Toolbar({ workflowId, onWorkflowSaved }: ToolbarProps) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { name: name || 'Untitled Workflow', nodes: data.nodes as WorkflowNode[], edges: data.edges };
      if (workflowId) {
  ...
  return (
    <div className="toolbar">
      ...
      <div className="toolbar-right">
        <button className="toolbar-btn" onClick={handleLoadTemplate} title="Load workflow template">
          模板
        </button>
        <button className="toolbar-btn primary" onClick={handleSave} disabled={saving}>
          {saving ? 'Saving...' : 'Save'}
        </button>
        <button className="toolbar-btn primary" onClick={handleRun} disabled={running || !workflowId}>
          {running ? 'Running...' : 'Run'}
        </button>
      </div>
    </div>
