...
export default function NodeConfigPanel({ nodeId, onChatMessage }: NodeConfigPanelProps) {
  ...
  const handleOpenInVSCode = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        onChatMessage({ role: 'assistant', content: '已在 VS Code 中打开: ' + String(filePath) });
      } else {
        onChatMessage({ role: 'assistant', content: '打开失败: ' + (data.error || '未知错误') });
      }
    } catch {
      onChatMessage({ role: 'assistant', content: 'VS Code API 不可用' });
    }
      ...
      <div className="panel-content">
        {nodeType === 'prompt' && (
          <div className="config-section">
            ...
            <div className="config-field">
              ...
              <textarea className="config-textarea" value={val('system_prompt')}
                onChange={(e) => syncField('system_prompt', e.target.value)}
                placeholder="系统角色设定、行为约束等..." rows={3} />
            </div>
            <div className="config-field">
              <label className="config-label">提示词模板</label>
              <textarea className="config-textarea" value={val('prompt')}
                onChange={(e) => syncField('prompt', e.target.value)}
                placeholder="输入提示词内容，可使用 {{变量}} 模板语法..." rows={4} />
            </div>
            <div className="config-section-title" style={{ marginTop: 16 }}>模型配置</div>
            <div className="config-field">
              <ModelSelector provider={val('provider')} modelName={val('model')} onChange={handleModelChange} />
            </div>
            <div className="config-field">
              <label className="config-label">API Key</label>
              <input className="config-input" type="password" value={val('api_key')}
                onChange={(e) => syncField('api_key', e.target.value)} placeholder="输入 API Key..." />
            </div>
          </div>
        )}
        ...
        {nodeType === 'code' && (
          <div className="config-section">
            ...
            <div className="config-field">
              ...
              <textarea className="config-textarea" value={val('description')}
                onChange={(e) => syncField('description', e.target.value)}
                placeholder="描述该代码节点的功能..." rows={3} />
            </div>
            <div className="config-field">
              <label className="config-label">文件路径</label>
              <input className="config-input" type="text" value={val('file_path')}
                onChange={(e) => syncField('file_path', e.target.value)}
                placeholder="如: output/analysis.py" />
            </div>
            <button style={{ width: '100%', padding: '8px 0', borderRadius: 6,
              border: '1px solid var(--border-color)', background: 'var(--bg-tertiary)',
              color: 'var(--text-primary)', cursor: 'pointer', fontSize: 13, marginTop: 8 }}
              onClick={handleOpenInVSCode} disabled={!val('file_path')}>
              在 VS Code 中打开
            </button>
          </div>
        )}
        ...
        {nodeType === 'output' && (
          <div className="config-section">
            ...
            <div style={{ background: 'var(--bg-tertiary)', borderRadius: 8, padding: 16,
              ...
              <p style={{ marginBottom: 8 }}>该节点将收集所有上游节点的输出结果。</p>
        ...
        {nodeType === 'start' && (
          <div className="config-section">
            ...
            <div style={{ background: 'var(--bg-tertiary)', borderRadius: 8, padding: 16,
              fontSize: 13, lineHeight: 1.6, color: 'var(--text-secondary)' }}>
              <p style={{ marginBottom: 8 }}>该节点是工作流的起点。</p>
              <p style={{ color: 'var(--text-muted)', fontSize: 12 }}>
